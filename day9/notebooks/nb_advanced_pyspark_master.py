...
# MAGIC ## How to run
# MAGIC
# MAGIC 1. Attach classic cluster `0703-105931-31juyffm`
# MAGIC 2. **Run all**
# MAGIC 3. Watch CMD counter — target ≥ 200 distinct APIs
# MAGIC 4. Charts appear via `display(fig)` / native `display(df)`

# COMMAND ----------

# MAGIC %md
# MAGIC ## 00 — Bronze (shared Cell 0)
...
# MAGIC %md
# MAGIC ## 01 — Complex types (struct / array / map)
# MAGIC
# MAGIC Nest FinLedger fields so one row carries a document.

# COMMAND ----------

# Build nested documents
nested = (
    base.limit(200)
    .withColumn(
        "payload",
...
# MAGIC %md
# MAGIC ## 02 — explode / posexplode / inline
# MAGIC
# MAGIC Unnest arrays without Python loops.

# COMMAND ----------

ex = nested.select("payload.id", F.explode("tags").alias("tag"))
cmd("explode")
...
# MAGIC %md
# MAGIC ## 03 — Higher-order functions (transform / filter / exists / aggregate)
# MAGIC
# MAGIC Spark SQL higher-order functions — Catalyst-native, not Python UDFs.
...
# MAGIC %md
# MAGIC ## 04 — Advanced windows (rowsBetween / rangeBetween / lag lead ntile)
# MAGIC
# MAGIC Moving averages and previous-day compare without collapsing grain.

# COMMAND ----------

w_ch = Window.partitionBy("channel_std").orderBy("txn_date", "transaction_id")
...
# MAGIC %md
# MAGIC ## 05 — Softening wide data (stack / melt pattern)
# MAGIC
# MAGIC Unpivot channel KPIs for chart-ready tall tables.
...
# MAGIC %md
# MAGIC ## 06 — Sampling / approx / sketch APIs
# MAGIC
# MAGIC Cheap exploration on large volumes.

# COMMAND ----------

s1 = base.sample(withReplacement=False, fraction=0.3, seed=42)
cmd("sample")
...
# MAGIC %md
# MAGIC ## 07 — Join repertoire (semi / anti / cross / using)
# MAGIC
# MAGIC Beyond inner/left — set membership joins.
...
# MAGIC %md
# MAGIC ## 08 — Set ops & schema evolve
# MAGIC
# MAGIC unionByName, intersect, except, with missing columns.

# COMMAND ----------

a = base.select("transaction_id", "amount", "channel_std").limit(50)
...
# MAGIC %md
# MAGIC ## 09 — Delta deep: history / detail / generate / vacuum dry-run
# MAGIC
# MAGIC Operate on a managed Delta table like production.

# COMMAND ----------

path_or_table = save_lab(base.limit(100).select("transaction_id", "amount", "channel_std", "txn_date"), "delta_ops")
...
# MAGIC %md
# MAGIC ## 10 — Delta MERGE advanced (matched delete / not matched by source)
# MAGIC
# MAGIC SCD-style and delete-via-merge patterns.
...
# MAGIC %md
# MAGIC ## 11 — Generated columns & partitioning & ZORDER
# MAGIC
# MAGIC Derived columns stored by Delta; file layout tips.
...
# MAGIC %md
# MAGIC ## 12 — Spark SQL power (CTE / QUALIFY / WINDOW / CUBE / ROLLUP)
# MAGIC
# MAGIC Analyst SQL that maps 1:1 to DataFrame.

# COMMAND ----------

base.createOrReplaceTempView("fin_base")
...
# MAGIC %md
# MAGIC ## 13 — Pandas / Arrow vectorized (mapInPandas optional)
# MAGIC
# MAGIC Bridge when libraries need pandas — batch, not row UDF.

# COMMAND ----------

pdf = base.limit(100).select("amount", "channel_std").toPandas()
cmd("toPandas")
print(pdf.describe())
...
# MAGIC %md
# MAGIC ## 14 — Broadcast join rates (Serverless-safe; no SparkContext)
# MAGIC
# MAGIC Fee enrichment via small rate dimension — DataFrame `broadcast()`, not `sparkContext`.

# COMMAND ----------

# Serverless cannot use spark.sparkContext.accumulator / .broadcast
...
# MAGIC %md
# MAGIC ## 15 — Repartition / coalesce / bucketBy / sortWithinPartitions
# MAGIC
# MAGIC Shuffle vs narrow dependency — file layout control.

# COMMAND ----------

# Prefer Spark SQL partition hint count — RDD APIs blocked on Serverless
try:
...
# MAGIC %md
# MAGIC ## 16 — Hinting & AQE observation
# MAGIC
# MAGIC Explain plans + adaptive execution (Serverless: conf keys may be blocked — soft-skip).

# COMMAND ----------

# Serverless often blocks spark.conf.get/set for AQE keys — never fail the cell
try:
...
# MAGIC %md
# MAGIC ## 17 — Streaming mindset (rate source → memory sink)
# MAGIC
# MAGIC Same DataFrame API; prove micro-batch locally.
...
# MAGIC %md
# MAGIC ## 18 — Databricks system tables (billing / access / compute)
# MAGIC
# MAGIC Soft-query UC system schemas; fall back to synthetic cost for charts.
...
# MAGIC %md
# MAGIC ## 19 — Cost dashboard visuals (matplotlib)
# MAGIC
# MAGIC Trainers: interpret DBU-like quantity over time — FinLedger teaching dashboard.
...
# MAGIC %md
# MAGIC ## 20 — Channel KPI dashboard (Databricks display + plots)
# MAGIC
# MAGIC Business visuals on FinLedger gold-style aggregates.
...
# MAGIC %md
# MAGIC ## 21 — dbutils filesystem & notebook utils
# MAGIC
# MAGIC Workspace ops trainers actually use.
...
# MAGIC %md
# MAGIC ## 22 — JSON / XML-ish / CSV options & schema enforcement
# MAGIC
# MAGIC Ingest edge cases beyond `inferSchema`.

# COMMAND ----------

js = nested.limit(10).select(F.to_json("payload").alias("json_doc"))
cmd("to_json")
...
# MAGIC %md
# MAGIC ## 23 — NaN / null advanced & masking
# MAGIC
# MAGIC Production hygiene for finance amounts.
...
# MAGIC %md
# MAGIC ## 24 — Catalog & information_schema inventory
# MAGIC
# MAGIC Governance-friendly listing of what this notebook created.

# COMMAND ----------

spark.sql("SHOW DATABASES").show(20)
cmd("SHOW DATABASES")
...
# MAGIC %md
# MAGIC ## 25 — Interview drill pack (dense unique APIs)
# MAGIC
# MAGIC Rapid-fire remaining verbs to clear the 200-command bar.
...
# MAGIC %md
# MAGIC ## 26 — USE CASE: Event Hubs — send FinLedger dummy pulses
# MAGIC
# MAGIC **Real-time path:** producer drops JSON onto Azure Event Hubs; Spark (or SDK) consumes.
# MAGIC
# MAGIC Analogy: Event Hub is a **conveyor belt** at the bank loading dock — many tellers drop payment slips; warehouse staff (Spark) pick them up in order.
# MAGIC
# MAGIC Prereq (once from laptop): `python scripts/ensure_eventhub_lab.py` → secrets on scope `finledger`.

# COMMAND ----------

# --- Event Hub producer (dummy FinLedger payments) ---
...
# MAGIC %md
# MAGIC ## 27 — USE CASE: Event Hubs — process pulses → silver KPI
# MAGIC
# MAGIC Consume from the hub (or the in-memory bus) → clean → aggregate → lab Delta + chart.
# MAGIC
# MAGIC Production next step on classic: `readStream.format("eventhubs")` with checkpoint on ADLS.
