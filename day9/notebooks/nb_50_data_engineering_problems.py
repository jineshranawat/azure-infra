...
# MAGIC - **K.** Medallion + SQL metadata + Purview capstone
# MAGIC
# MAGIC ## Full index
# MAGIC
...
# MAGIC %md
# MAGIC # Connectivity & Key Classes Primer — Teach This BEFORE the 50 Scenarios
# MAGIC
# MAGIC **Why this belongs before Problem 01:** Every one of the 50 scenarios starts from `bronze` already being a DataFrame. But `bronze` didn't appear by magic — cell 3 runs a three-method authentication fallback chain to physically pull `sample_transactions.csv` out of ADLS Gen2 and into Spark. For a 10–15 year audience, skipping *how the data got in* is skipping the most transferable, most interview-relevant, most production-critical part of the whole notebook. The scenarios teach Spark APIs; this primer teaches the connectivity and object model those APIs sit on top of.
# MAGIC
# MAGIC Teach this as a standalone 20–30 min segment, walking through cell 3 line by line, before anyone touches Problem 01.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 0 — The mental model to open with
# MAGIC
# MAGIC There are **two fundamentally different ways** to read data from ADLS in this notebook, and understanding the distinction is the whole point of this segment:
# MAGIC
# MAGIC 1. **Distributed read (the cluster reads):** `spark.read...csv(path)` — every executor in the cluster independently reads its share of the file(s) directly from ADLS, in parallel. The driver never sees the raw bytes. This is how you read production-scale data. Methods 1 and 2 in the code both do this.
# MAGIC
# MAGIC 2. **Driver-local read (one machine reads):** `DataLakeServiceClient` — a single Python process on the *driver* downloads the entire file into the driver's memory as a byte string, then hands it to Spark. This does not scale (it's bounded by one machine's memory and one network pipe), but it works in restricted environments where the distributed path is blocked. Method 3 does this.
# MAGIC
# MAGIC **The line to open with:** "The reason this cell has three methods isn't defensive over-engineering — it's that the *right* way to read (let the cluster do it) is exactly the way that gets blocked in locked-down enterprise environments, so we need a fallback that trades scalability for the ability to work at all."
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 1 — The `abfss://` path and the connectivity model underneath
# MAGIC
# MAGIC Before any class, the audience needs to understand what this path actually means:
# MAGIC
# MAGIC ```
# MAGIC abfss://bronze@stsharedqgr7mj.dfs.core.windows.net/loaded/run=session3-lab/sample_transactions.csv
# MAGIC         └────┘ └────────────┘└──────────────────┘ └──────────────────────────────────────────┘
# MAGIC         container  account         endpoint                        path within container
# MAGIC ```
# MAGIC
# MAGIC **Theory to explain:**
# MAGIC
# MAGIC - **`abfss`** = Azure Blob File System Secure. It's the driver/protocol scheme Spark's Hadoop layer uses to talk to ADLS Gen2 over TLS. The `s` matters — `abfs` (no TLS) exists but you should never see it in production.
# MAGIC - **`.dfs.core.windows.net`** is the **Data Lake Storage Gen2 (hierarchical namespace) endpoint**. This is a critical distinction to draw for a senior room: the *same* storage account exposes two different endpoints — `.blob.core.windows.net` (flat blob namespace, the classic object store) and `.dfs.core.windows.net` (hierarchical namespace, real directories with atomic rename/delete). ADLS Gen2 is not a separate product — it's a blob storage account with the **Hierarchical Namespace (HNS)** feature flag enabled, which is what gives you POSIX-like directory semantics that make operations like "rename a folder" or "list a directory" atomic and cheap instead of an O(n) copy-and-delete over every blob with that prefix.
# MAGIC - **`bronze@...`** — the `container@account` syntax. In ADLS Gen2 terminology a container is called a "file system," which is why the SDK method is literally `get_file_system_client("bronze")`.
# MAGIC
# MAGIC **Why HNS matters for the 50 problems (connect it forward):** Delta Lake's transaction guarantees, atomic commits, and the OPTIMIZE/VACUUM file operations (Problems 05, 47, 49) all rely on ADLS Gen2's atomic rename being cheap. On flat blob storage without HNS, a Delta "commit" that renames a temp file would be a non-atomic copy — which is exactly the class of subtle correctness bug that makes people say "just use Gen2 for lakehouse workloads."
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 2 — `DataLakeServiceClient`: the class, its hierarchy, and its main methods
# MAGIC
# MAGIC This is the specific class the student asked about. The critical thing to teach is that it's the **top of a four-level client hierarchy** that mirrors the storage structure itself:
# MAGIC
# MAGIC ```
# MAGIC DataLakeServiceClient          → the whole storage account
# MAGIC   └─ FileSystemClient          → one container ("bronze")
# MAGIC        └─ DataLakeDirectoryClient → a directory within it (optional)
# MAGIC        └─ DataLakeFileClient     → one specific file
# MAGIC ```
# MAGIC
# MAGIC You *navigate down* this hierarchy — you don't construct the lower clients directly, you ask the level above for them. That's why the code reads like a chain: service → filesystem → file.
# MAGIC
# MAGIC ### The main methods, grouped by what they're for
# MAGIC
# MAGIC **Construction / connection (on `DataLakeServiceClient`):**
# MAGIC
# MAGIC | Method | What it does | When you'd use it |
# MAGIC |---|---|---|
# MAGIC | `DataLakeServiceClient(account_url, credential)` | The constructor — establishes the account-level client. `account_url` points at the `.dfs.` endpoint; `credential` is the auth (see Part 4). | Every time. This is the entry point. |
# MAGIC | `DataLakeServiceClient.from_connection_string(conn_str)` | Alternative constructor from a single connection string (bundles account + key). | Convenient for scripts, but discouraged — it embeds the key in one string that's easy to leak (ties to Problem 31). |
# MAGIC
# MAGIC **Navigation (getting to lower-level clients):**
# MAGIC
# MAGIC | Method | Returns | Notes |
# MAGIC |---|---|---|
# MAGIC | `.get_file_system_client("bronze")` | `FileSystemClient` | The container-level handle. Cheap — it doesn't make a network call, just constructs a scoped client. |
# MAGIC | `.get_directory_client(path)` | `DataLakeDirectoryClient` | For directory-level operations (create, rename, delete, set permissions). |
# MAGIC | `.get_file_client(rel_path)` | `DataLakeFileClient` | The handle to one specific file. Also cheap/lazy — no I/O until you call a data method on it. |
# MAGIC
# MAGIC **Account-level operations (less used in a read path, but worth naming):**
# MAGIC
# MAGIC | Method | What it does |
# MAGIC |---|---|
# MAGIC | `.list_file_systems()` | Enumerate containers in the account. |
# MAGIC | `.create_file_system(name)` / `.delete_file_system(name)` | Container lifecycle. |
# MAGIC | `.get_service_properties()` | Account-level config (logging, CORS, etc.). |
# MAGIC
# MAGIC **The actual data movement (on `DataLakeFileClient`):**
# MAGIC
# MAGIC | Method | What it does | Critical detail for seniors |
# MAGIC |---|---|---|
# MAGIC | `.download_file()` | Returns a `StorageStreamDownloader` — a **lazy handle** to the download, not the bytes yet. | This is the key nuance: `download_file()` alone hasn't pulled the data. It sets up a streaming download. |
# MAGIC | `.readall()` | Pulls the **entire file** into memory as `bytes`. | This is where the actual network transfer happens, and where a large file will OOM your driver. It's the non-scalable step. |
# MAGIC | `.read(offset, length)` | Range read — pull a specific byte range. | The scalable-ish alternative when you only need part of a file. |
# MAGIC | `.upload_data(data, overwrite=)` | Write path (not used here, but the symmetric operation). | For completeness — this class writes too, not just reads. |
# MAGIC
# MAGIC ### Now walk the actual code, method by method:
# MAGIC
# MAGIC ```python
# MAGIC client = DataLakeServiceClient(
# MAGIC     account_url=f"https://{account}.dfs.core.windows.net",  # account-level, .dfs endpoint
# MAGIC     credential=key,                                          # the storage account key (Part 4)
# MAGIC )
# MAGIC raw = (
# MAGIC     client
# MAGIC     .get_file_system_client("bronze")   # → FileSystemClient scoped to the 'bronze' container
# MAGIC     .get_file_client(rel)               # → DataLakeFileClient for loaded/run=.../sample_transactions.csv
# MAGIC     .download_file()                    # → StorageStreamDownloader (LAZY — no bytes moved yet)
# MAGIC     .readall()                          # → bytes: NOW the full file transfers into driver memory
# MAGIC     .decode("utf-8")                    # → str: interpret those raw bytes as UTF-8 text
# MAGIC )
# MAGIC ```
# MAGIC
# MAGIC **The teaching beat:** point out that `get_file_system_client` and `get_file_client` are *free* (no network call — they just build scoped client objects), `download_file()` is *lazy* (sets up the stream), and `readall()` is the one line that actually moves every byte across the network into the driver's RAM. This laziness pattern — cheap handle construction, deferred I/O — is the same principle as Spark's lazy DataFrames, and worth calling out as a recurring design pattern rather than a coincidence.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 3 — The `raw` object and its options (what the student asked as Q2)
# MAGIC
# MAGIC `raw` at each stage is a **different type**, and that progression is the whole point:
# MAGIC
# MAGIC ```
# MAGIC .download_file()  →  StorageStreamDownloader   (a lazy download handle)
# MAGIC .readall()        →  bytes                       (raw binary content of the file)
# MAGIC .decode("utf-8")  →  str                         (the CSV as one big text string)
# MAGIC ```
# MAGIC
# MAGIC So the final `raw` is a **single Python `str` containing the entire CSV**, newlines and commas and all — literally the file's text sitting in the driver's memory.
# MAGIC
# MAGIC **The options on this object worth explaining:**
# MAGIC
# MAGIC - **`.readall()` vs `.read(offset, length)`** — `readall()` is all-or-nothing (whole file → memory); `read()` lets you pull a byte range, which matters if you ever adapt this for a file too big to fully materialize.
# MAGIC - **`.decode("utf-8")`** — this is where encoding assumptions live. If the source file were Latin-1 or had a BOM, this line is where it'd break or silently corrupt. For a senior audience, flag that hardcoding `"utf-8"` is a reasonable default but an *assumption* — real ingestion often needs `.decode("utf-8-sig")` to strip a BOM, or explicit encoding detection. This is a genuine, common production bug source.
# MAGIC - **`.readinto(stream)`** — an alternative that reads into a pre-allocated buffer/stream rather than returning a new bytes object, avoiding a memory copy for large downloads.
# MAGIC
# MAGIC **The trade-off to name explicitly:** the entire method-3 path exists in driver memory as one string. That's fine for a training CSV of a few thousand rows. It is catastrophic for a multi-GB file — you'd exhaust the driver heap at `readall()`. This is precisely why methods 1 and 2 (distributed cluster reads) are tried *first*, and method 3 is the last-resort fallback with an explicit comment saying "small training CSV — OK for lab."
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 4 — Authentication: the three-method chain and why it exists
# MAGIC
# MAGIC This is the highest-value part for a senior audience, because it's really a tour of **how Azure identity and storage access actually work**, which transfers to almost any Azure data engineering role.
# MAGIC
# MAGIC ### Method 1 — Storage account key in Spark's Hadoop config
# MAGIC
# MAGIC ```python
# MAGIC spark.conf.set(f"fs.azure.account.key.{account}.dfs.core.windows.net", key)
# MAGIC frame = spark.read.option("header", True).option("inferSchema", True).csv(path)
# MAGIC ```
# MAGIC
# MAGIC **Theory:** This injects the storage account's shared key into Spark's Hadoop filesystem configuration. Every executor picks up this config and uses the key to authenticate its own direct reads from ADLS. It's a **distributed read** — the cluster does the work in parallel, driver never touches raw bytes.
# MAGIC
# MAGIC **Why it's tried first:** it's the fastest and most scalable path.
# MAGIC
# MAGIC **Why it fails (and the code falls through):** the shared account key is the "root password" of the storage account — full access, no scoping, no expiry, no per-user audit trail. Serverless compute and Unity-Catalog-governed clusters **deliberately block `spark.conf.set` for account keys**, because allowing arbitrary notebook code to set a root-level credential defeats the entire governance model. This is a security *feature*, not a limitation — and it's exactly why the code wraps it in try/except and moves on.
# MAGIC
# MAGIC ### Method 2 — Azure credential passthrough (RBAC)
# MAGIC
# MAGIC ```python
# MAGIC frame = spark.read.option("header", True).option("inferSchema", True).csv(path)
# MAGIC # (no key set — relies on the cluster's own identity)
# MAGIC ```
# MAGIC
# MAGIC **Theory:** No credential in the notebook at all. The read succeeds if the **cluster's own managed identity / the user's Azure AD (Entra ID) identity** has been granted an RBAC role (like `Storage Blob Data Reader`) on the storage account. Authentication is delegated to Azure's identity plane — Spark authenticates *as whoever the cluster is running as*, and Azure checks that identity's role assignments.
# MAGIC
# MAGIC **Why this is the "right" enterprise pattern:** no secret ever touches the notebook (solves Problem 31 by construction), access is per-identity and auditable, and it's scoped by RBAC role rather than being all-or-nothing. This is what a well-governed Databricks + Unity Catalog environment actually uses.
# MAGIC
# MAGIC **Why it might still fail (fall through to method 3):** if the cluster identity hasn't been granted the RBAC role, or the environment is Spark Connect / a restricted serverless mode where even this path is constrained, the distributed read fails and we hit the last resort.
# MAGIC
# MAGIC ### Method 3 — Driver-side SDK (the `DataLakeServiceClient` path)
# MAGIC
# MAGIC The fallback we dissected in Parts 2–3. **Why it's last:** it doesn't scale (single-driver, whole-file-in-memory), and it puts the key back in the notebook process. It exists purely so the lab *works at all* in the most locked-down environments where both distributed paths are blocked — trading every good property (scale, security, audit) for basic functionality.
# MAGIC
# MAGIC ### The one slide that ties Part 4 together
# MAGIC
# MAGIC | Method | Who reads | Credential | Scales? | Governed? | Why in this position |
# MAGIC |---|---|---|---|---|---|
# MAGIC | 1. Spark conf key | Cluster (distributed) | Account key in Spark conf | ✅ | ❌ (root key) | Fast, tried first; blocked by Serverless/UC |
# MAGIC | 2. RBAC passthrough | Cluster (distributed) | Cluster/user identity | ✅ | ✅ (best) | The "right" way; needs role granted |
# MAGIC | 3. Driver SDK | Driver (single node) | Account key in SDK client | ❌ | ❌ | Last resort; works when 1+2 blocked |
# MAGIC
# MAGIC **The closing line for Part 4:** "This fallback chain is a map of the tension every enterprise data platform lives with — the most scalable, secure method requires the most setup and governance, so a robust ingest path degrades gracefully from 'ideal' to 'at least it runs,' and tells you which rung it landed on via `BRONZE_AUTH_MODE`."
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## Part 5 — `spark.createDataFrame(pd.read_csv(StringIO(raw)))` (the student's Q3)
# MAGIC
# MAGIC This is the line that converts the driver-local string back into a distributed Spark DataFrame. It's a three-stage transformation and each stage matters:
# MAGIC
# MAGIC ```python
# MAGIC spark.createDataFrame( pd.read_csv( StringIO(raw) ) )
# MAGIC                                      └──────────┘
# MAGIC                                      (1)
# MAGIC                        └──────────────────────┘
# MAGIC                        (2)
...
# MAGIC %md
# MAGIC ## Part K — Medallion + SQL metadata + Purview capstone
# MAGIC
# MAGIC After all **50 problems**, this section ties every `prob*` output to:
# MAGIC
# MAGIC 1. **Medallion pipeline** — bronze → silver Delta → gold aggregates on ADLS  
# MAGIC 2. **SQL metadata model** — `de_table_registry.csv` on `audit/governance/` for ADF/Azure SQL  
# MAGIC 3. **Purview governance** — manifest JSON with glossary, classifications, lineage, and all 50 problem assets  
# MAGIC
# MAGIC **ADF integration:** `shared-adf-lab` folder `13-governance-purview` (`pl_gov_06`) reads the same audit paths.
# MAGIC
# MAGIC **CI/CD:** `release-medallion-governance.cmd` from repo root builds, deploys, and seeds SQL registry.
