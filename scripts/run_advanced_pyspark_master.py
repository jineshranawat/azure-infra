#!/usr/bin/env python3
"""Import + run advanced PySpark master notebook on shared Databricks (classic)."""
...
BUILD = REPO_ROOT / "day9" / "scripts" / "build_advanced_pyspark_notebook.py"
NOTEBOOK_SRC = REPO_ROOT / "day9" / "notebooks" / "nb_advanced_pyspark_master.py"
NOTEBOOK_PATH = "/Shared/day9/advanced_pyspark_master"
JOB_NAME = "advanced-pyspark-master"
...
def _request(host: str, token: str, method: str, path: str, body: dict | None = None) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Databricks API {method} {path} failed ({e.code}): {e.read().decode()[:800]}") from e
...
def _resolve_host(file_env: dict[str, str]) -> str:
    ...
    az = shutil.which("az")
    if not az:
        raise SystemExit("az CLI not found and DATABRICKS_HOST missing")
    data = json.loads(
...
def main() -> int:
    ...
    for _ in range(120):
        ...
        if life == "TERMINATED":
            ...
            print()
            print("=" * 70)
            print("ADVANCED PYSPARK MASTER — RUN COMPLETE")
            print("=" * 70)
